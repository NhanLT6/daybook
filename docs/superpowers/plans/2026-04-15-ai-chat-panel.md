...
**Tech Stack:** `ai` + `@ai-sdk/google` (Vercel AI SDK with Google provider), `@vercel/kv` (Upstash Redis), Web Crypto API (ECDSA P-256), Vitest (unit tests), Vuetify `v-navigation-drawer`, Vue 3 Composition API.
...
```bash
yarn add ai @ai-sdk/google @vercel/kv
```

- `ai` — Vercel AI SDK core (`generateText`, `streamText`, `CoreMessage` types)
- `@ai-sdk/google` — Google Gemini provider for the AI SDK (`createGoogleGenerativeAI`)
- `@vercel/kv` — Upstash Redis KV store

Expected: all three packages appear in `dependencies` in `package.json`.
...
Uses the Vercel AI SDK (`ai` + `@ai-sdk/google`) so switching to a different provider later is a one-line change.

...
<template>
  <div :class="['d-flex ga-2', message.role === 'user' ? 'justify-end' : 'justify-start']">
    ...
    <!-- Message card -->
    <!-- User: tonal primary (matches tonal buttons). AI: surface with elevation (matches log table). -->
    <VCard
      :color="message.role === 'user' ? 'primary' : 'container'"
      :variant="message.role === 'user' ? 'tonal' : 'elevated'"
      :elevation="0"
    ...
    >
      <VCardText class="pa-3">
        <!-- Image attachment: position relative + z-index raises it above the tonal card overlay -->
        <VImg
        ...
        <template v-if="message.extractedLogs?.length">
          ...
          <div class="d-flex ga-2 justify-end mt-3">
            <VBtn size="small" variant="text" color="default" :disabled="!isSaveable"> Discard </VBtn>
